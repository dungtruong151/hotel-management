...
function createData(reservation_id, name, room_number, total_amount, amount_paid, status, actions) {
  return { reservation_id, name, room_number, total_amount, amount_paid, status, actions };
}
...
export default function DataTableGuest({ searchValue }) {
  ...
  useEffect(() => {
    ...
    if (isNaN(searchValue)) {
      ...
      axios.get(`http://localhost:5000/${searchValue}`)
      .then((response) => {
        setData(response.data);
        rows = [];
  ...
  const handleUpdate = async () => {
      ...
      try {
          const response = await axios.put(`http://localhost:5000/update/${selectedId}`, guestData);
                  ...
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code} >
                    {columns.map((column) => {
                      ...
                      if (column.id === 'reservation_id') {
                        value = '#' + value;
                      } else if (column.id === 'total_amount' || column.id === 'amount_paid') {
                        value = '$ ' + value;
                      }
                      return (
                        <TableCell 
                          ...
                          sx={{color: grey[600]}}
                          style={{
                            ...column.id === 'reservation_id' ? { fontWeight: 'bold' } : {},
                          }}
                        >
                          {value === null ? 
                          <Box>
                              <Tooltip title="Open settings">
                                <IconButton onClick={(event) => handleOpenUserMenu(event, row.reservation_id)} sx={{ p: 0 }}>
                                    <MoreVertIcon />
                                </IconButton>
                              </Tooltip>
                              <Menu
